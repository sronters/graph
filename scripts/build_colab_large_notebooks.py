    ...
    return {
        ...
        "cells": [
            ...
            code(
                ...
                "print({'profile': PROFILE, 'colab_release': os.getenv('COLAB_RELEASE_TAG'), "
                "'python': sys.version, 'platform': platform.platform(), 'cpus': os.cpu_count()})\n"
            ),
            ...
            code(
                ...
                "    subprocess.run(['uv', 'run', 'graphtrust', 'generate', '--profile', PROFILE, "
                "'--scale', 'large', '--seed', str(SEED), '--variants', 'injected_mixed'], check=True)\n"
                "print(dataset)\n"
            ),
            ...
            code(
                ...
                "archive = shutil.make_archive(f'/content/graphtrust-large-{PROFILE}-{SEED}', 'zip', 'artifacts/large_runs')\n"
                "print({'archive': archive, 'verified_profiles': len(receipt_data['profiles'])})\n"
