...
def notebook(profile: str, label: str) -> dict[str, object]:
    return {
        ...
        "cells": [
            ...
            markdown(
                "## 0. Settings and Kaggle workspace\n\n"
                "Enable **Internet** in the Kaggle notebook settings. If Internet is unavailable, "
                "attach a Kaggle Dataset mounted at `/kaggle/input/graphtrust-source` containing "
                "`GraphTrust_NSRI_Main_Source.zip` (or `GraphTrust_Kaggle_Source.zip`) and "
            ...
            code(
                ...
                "SOURCE_INPUT = Path('/kaggle/input/graphtrust-source')\n"
                "EXPECTED_COMMIT = '213bcc730f3fa1702cc0c78f0c9b9f2b5810b2a1'\n"
            ...
            code(
                ...
                "SOURCE_MODE = 'existing_git' if (ROOT / '.git').exists() else ''\n"
                "if not (ROOT / '.git').exists():\n"
                "    source_archives = [SOURCE_INPUT / 'GraphTrust_NSRI_Main_Source.zip', SOURCE_INPUT / 'GraphTrust_Kaggle_Source.zip']\n"
                "    source_archive = next((path for path in source_archives if path.exists()), None)\n"
                "    source_tree = SOURCE_INPUT / 'pyproject.toml'\n"
                "    if source_archive is not None:\n"
                "        ROOT.mkdir(parents=True, exist_ok=True)\n"
                "        shutil.unpack_archive(str(source_archive), str(ROOT))\n"
                "        SOURCE_MODE = 'kaggle_input_archive'\n"
                "    elif source_tree.exists():\n"
                "        shutil.copytree(SOURCE_INPUT, ROOT, dirs_exist_ok=True)\n"
                "        SOURCE_MODE = 'kaggle_input_tree'\n"
                "    else:\n"
                "        try:\n"
                "            subprocess.run(['git', 'clone', '--depth', '1', '--branch', BRANCH, REPO, str(ROOT)], check=True)\n"
                "            SOURCE_MODE = 'git_clone'\n"
                "        except subprocess.CalledProcessError as error:\n"
                "            raise RuntimeError('GitHub is unreachable. Enable Kaggle Internet or attach the graphtrust-source dataset described above.') from error\n"
                "os.chdir(ROOT)\n"
                "if (ROOT / '.git').exists():\n"
                ...
                "else:\n"
                "    marker = SOURCE_INPUT / 'GRAPHTRUST_SOURCE_COMMIT.txt'\n"
                "    COMMIT = marker.read_text(encoding='utf-8').strip() if marker.exists() else EXPECTED_COMMIT\n"
                "    assert COMMIT == EXPECTED_COMMIT, f'Offline source commit must be {EXPECTED_COMMIT}, got {COMMIT}'\n"
                "    SOURCE_MODE = globals().get('SOURCE_MODE', 'kaggle_input_tree')\n"
                "subprocess.run([sys.executable, '-m', 'pip', 'install', '-q', 'uv'], check=True)\n"
                "subprocess.run(['uv', 'sync', '--frozen', '--all-extras'], check=True)\n"
                "os.environ['GRAPHTRUST_SOURCE_COMMIT'] = COMMIT\n"
            ...
            code(
                ...
                "package_manifest = {'schema_version': '1.0', 'profile': PROFILE, 'seed': SEED, 'git_commit': COMMIT, 'dataset_id': profile_receipt['dataset_id'], 'dataset_checksum': profile_receipt['dataset_checksum'], 'run_id': profile_receipt['run_id'], 'execution_platform': 'kaggle', 'files': {}}\n"
                ...
                "download_receipt = {'archive': output_archive.name, 'archive_sha256': archive_sha256, 'package_manifest_sha256': package_manifest_digest, 'git_commit': COMMIT, 'profile': PROFILE, 'seed': SEED, 'dataset_id': profile_receipt['dataset_id'], 'dataset_checksum': profile_receipt['dataset_checksum'], 'run_id': profile_receipt['run_id'], 'execution_platform': 'kaggle', 'source_mode': SOURCE_MODE}\n"
